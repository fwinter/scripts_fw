...
my $meson_db = $HMCParam::meson_file;

my $local_meson_db = "/scratch/${meson_db}";
...
#&print_read_single_lime_colorvec($eig_db, $input);

&print_meson_colorvec_mom_dispX("$local_meson_db", $input);
...
&copy_back($HMCParam::remote_meson_db, $local_meson_db);
