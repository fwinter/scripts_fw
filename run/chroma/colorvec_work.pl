...
##  my $err = 0xffff & system("modkeys $file > /dev/null");
  my $err = 0xffff & system("/home/edwards/bin/exe/cuda/modkeys_centos55 $file > /dev/null");
  if ($err > 0x00)
  {
...
# -- nonzero momentum and ONLY two displacements
#
sub print_meson_colorvec_mom_disp3only
{
  local($db_file, $input) = @_;
  open(INPUT, ">> $input");
  print INPUT <<EOF;
...
sub print_meson_colorvec_mom_dispX
{
  local($db_file, $input) = @_;
#  local($db_file, $input, $mom2_min, $mom2_max, $deriv_min, $deriv_max, @mom_list) = @_;
  open(INPUT, ">> $input");
  print INPUT <<EOF;
    <elem>
      <Name>MESON_MATELEM_COLORVEC</Name>
      ...
      <Param>
        ...
        <mom2_min>${HMCParam::mom2_min}</mom2_min>
        <mom2_max>${HMCParam::mom2_max}</mom2_max>
        <start_t>${HMCParam::start_t}</start_t>
        <end_t>${HMCParam::end_t}</end_t>
...
EOF
  ...
  for (my $nderiv = ${HMCParam::deriv_min}; $nderiv <= ${HMCParam::deriv_max}; $nderiv++)
  {
    &print_deriv_list($input, $nderiv);
  }
  open(INPUT, ">> $input");
  print INPUT <<EOF;
        </deriv_list>
...
EOF
  close(INPUT);
  &print_mom_list($input, @{HMCParam::mom_list});
  open(INPUT, ">> $input");
  print INPUT <<EOF;
        </mom_list>
...
#------------------------------------------------------------------------------
# Print meson colorvec matrix elements
# -- nonzero momentum and derivatives
#
sub print_meson_deriv_colorvec_mom
{
  local($db_file, $input, $mom2_min, $mom2_max, $deriv_min, $deriv_max, @mom_list) = @_;
  open(INPUT, ">> $input");
  print INPUT <<EOF;
...
EOF
  close(INPUT);
  for (my $nderiv = $deriv_min; $nderiv <= $deriv_max; $nderiv++)
  {
    &print_deriv_list($input, $nderiv);
  }
  open(INPUT, ">> $input");
  print INPUT <<EOF;
      ...
      <NamedObject>
        ...
        <colorvec_files>
EOF
  foreach my $ee (@{HMCParam::eig_files})
  {
    printf INPUT "          <elem>${ee}</elem>\n";
  }
...
# -- nonzero momentum and derivatives
#
sub print_meson_deriv_colorvec_mom2
{
  local($db_file, $input, $mom2_min, $mom2_max, $deriv_min, $deriv_max, @mom_list) = @_;
  open(INPUT, ">> $input");
  print INPUT <<EOF;
...
#
sub print_unitary_rot
{
  local($db_file, $input) = @_;
  open(INPUT, ">> $input");
  print INPUT <<EOF;
    <elem>
      <Name>CONSTRUCT_EGG_COLORVEC</Name>
      <Frequency>1</Frequency>
      <Param>
        <version>1</version>
        <coord_type>${HMCParam::coord_type}</coord_type>
        <num_vecs>${HMCParam::num_vecs}</num_vecs>
        <embed_size>${HMCParam::embed_size}</embed_size>
        <dt>${HMCParam::dt}</dt>
        <tol>${HMCParam::tol}</tol>
        <decay_dir>3</decay_dir>
      </Param>
      <NamedObject>
        <colorvec_files><elem>${HMCParam::eig_file}</elem></colorvec_files>
        <rot_file>${db_file}</rot_file>
      </NamedObject>
    </elem>
...
sub print_glue_dist_colorvec
{
  ...
  print INPUT <<EOF;
    <elem>
      <Name>GLUEBALL_DIAG_MATELEM_COLORVEC</Name>
      <Frequency>1</Frequency>
      <Param>
        <version>1</version>
        <mom2_max>${HMCParam::mom2_max}</mom2_max>
...
#------------------------------------------------------------------------
# Print list of momenta for xml
sub print_mom_list
{
  local($input, @mom_list) = @_;
  my $num_mom = @HMCParam::mom_list;

  open(INPUT, ">> $input");
  foreach $i (0 .. ($num_mom-1) )
  {
    print INPUT "    <elem>$mom_list[$i]</elem>";
  }
  close(INPUT);
...
#------------------------------------------------------------------------
# Print list of derivatives for xml
sub print_deriv_list
{
  local($input, $num_deriv) = @_;

  open(INPUT, ">> $input");
  if ($num_deriv == 0)
  {
    print INPUT "          <elem></elem>\n";
  }
  elsif ($num_deriv == 1)
  {
    print INPUT <<EOF;
...
EOF
  ...
  {
    print "Unsupported number of derivatives = ${num_deriv}\n";
    exit(1);
  }
  close(INPUT);
